...
class Trigon:
    def __init__(self, *args):
        ...
        args_list = []  # сюда пишем аргументы
        for i in args:  # цикл для вычитывания аргументов
            if not isinstance(i, int):  # если i не число выведи ошибку
                raise TypeError('Стороны должны быть числами')
            if i <= 0:  # если i меньше 0 выведи ошибку
                raise ValueError('Стороны должны быть положительными')
            args_list.append(i)  # добавить аргумент в список
        if len(args_list) != 3:  # если в списке не 3 аргумента, ошибка
            raise IndexError(f'Передано {len(args_list)} аргументов, а ожидается 3')
        else:  # иначе проверяем треугольник ли это. !!!Можно сделать через or
            if args_list[0] + args_list[1] < args_list[2]:
                raise Exception('Не треугольник')
            elif args_list[1] + args_list[2] < args_list[0]:
                raise Exception('Не треугольник')
            elif args_list[0] + args_list[2] < args_list[1]:
                raise Exception('Не треугольник')
