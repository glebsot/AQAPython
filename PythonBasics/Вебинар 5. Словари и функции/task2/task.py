...
def repeats(our_str):
    ...
    dict1 = {}
    str1 = ''
    for i in our_str:  # перебираем все буквы строки
        if i in dict1:  # если ключ есть в словаре, то к его значению прибавляем 1,
            # т.е. считаем какое по счету это повторение
            dict1[i] += 1
        else:  # в остальных случаях пишем ключ с дефолтным значение 1
            dict1.setdefault(i, 1)
        str1 += f'{i}_{dict1[i]}'  # формируем строку по тз
    return str1
