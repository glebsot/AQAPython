...
class PersonInfo:
    ...
    def short_name(self):
        ...
        fi_list = self.fi.split(' ')
        return f'{fi_list[1]} {fi_list[0][0]}.'
    ...
    def path_deps(self):
        ...
        for i in self.deps:
            part_deps += f'{i} --> '
        return ''.join(part_deps[:-5])

    def new_salary(self):
        """Новая зарплата"""
        # todo Здесь нужно написать код
        part_deps = ''
        for i in self.deps:  # создаём строку со всеми буквами в названиях подразделений
            part_deps += i
        dict_letter = {i: part_deps.count(i) for i in part_deps}
        # создаём словарь с буквами и их вхождениями в строку part_deps
        values_list = []
        values_list.extend(dict_letter.values())  # создаём список количества вхождений букв
        sum_of_occ = 0
        for i in range(3):  # суммируем три самые большие цифры из списка вхождений
            sum_of_occ += max(values_list)
            values_list.remove(max(values_list))
        return 1337 * self.age * sum_of_occ
