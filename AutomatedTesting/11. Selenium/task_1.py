...
def test_news_tensor(start_driver):
    ...
    contacts.click()
    link_office = WebDriverWait(start_driver, 10).until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, '[href = "/contacts"] span')))
    link_office.click()
    ...
    logo_tensor = WebDriverWait(start_driver, 10).until(
        EC.visibility_of_element_located((By.CSS_SELECTOR, '.sbisru-Contacts__logo-tensor')))
    logo_tensor.click()
    ...
    news_block4 = WebDriverWait(start_driver, 10).until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, '.tensor_ru-Index__block4-bg [href="/about"]'))
    )
